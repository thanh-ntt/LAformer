...
class ModelMain(nn.Module):
    ...
    def forward(self, mapping: List[Dict], device):
        ...
        agents_lanes_embed, lane_states_batch = self.encoder.forward(mapping, vector_matrix, polyline_spans, device, batch_size)
        # Global interacting graph
        inputs, inputs_lengths = utils.merge_tensors(agents_lanes_embed, device=device)
